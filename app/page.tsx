...
type Stage = {
  ...
  skills: Skill[][]; gear: Gear[]; notes: string; image?: string;
};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) { const parsed = JSON.parse(saved); if (Array.isArray(parsed) && parsed.length) { setGuides(parsed); setGuideId(parsed[0].id); setStageId(parsed[0].stages[0].id); } }
  ...
  const progress = useMemo(() => {
    if (!guide) return 0;
    const keys = guide.stages.flatMap(s => [...s.goals, ...s.conditions].map((_, i) => `${guide.id}-${s.id}-${i}`));
  ...
  const exportData = () => {
    const blob = new Blob([JSON.stringify({ format:"poe-guide-v1", guides }, null, 2)], { type:"application/json" });
  ...
  const importData = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const reader = new FileReader(); reader.onload = () => { try { const raw = JSON.parse(String(reader.result)); const next = Array.isArray(raw) ? raw : raw.guides; if (!Array.isArray(next) || !next.length) throw new Error(); setGuides(next); setGuideId(next[0].id); setStageId(next[0].stages[0].id); } catch { alert("無法讀取：請選擇由本網站匯出的攻略 JSON。"); } }; reader.readAsText(file); e.target.value = "";
  };
  const newGuide = () => {
  ...
  return (
    <main className="app-shell">
      ...
      <aside className="sidebar">
        ...
        <nav aria-label="攻略階段">{guide.stages.map((s, i) => <button key={s.id} className={s.id===stage.id?"stage active":"stage"} onClick={() => setStageId(s.id)}><span className="stage-number">{String(i).padStart(2,"0")}</span><span>{s.name}</span><i>{s.goals.filter((_,x)=>checked[`${guide.id}-${s.id}-${x}`]).length}/{s.goals.length}</i></button>)}</nav>
      ...
      <section className="workspace">
        ...
        <div className="dashboard">
          <article className="card goals-card"><div className="card-heading"><span className="icon">◎</span><div><small>STEP {stageIndex+1}</small><h2>目前目標</h2></div></div>
            {editing ? <><textarea value={stage.summary} onChange={e=>patchStage({summary:e.target.value})}/><label className="field-label">每行一個目標</label><textarea value={stage.goals.join("\n")} onChange={e=>patchStage({goals:textList(e.target.value)})}/></> : <><p className="summary">{stage.summary}</p><ul className="checklist">{stage.goals.map((g,i)=>{const k=`${guide.id}-${stage.id}-${i}`;return <li key={k}><label><input type="checkbox" checked={!!checked[k]} onChange={()=>setChecked(c=>({...c,[k]:!c[k]}))}/><span>{g}</span></label></li>})}</ul></>}
          </article>
          ...
          <article className="card conditions-card"><div className="card-heading"><span className="icon">✓</span><div><small>READY CHECK</small><h2>轉型條件</h2></div></div>
            {editing ? <><label className="field-label">每行一個條件</label><textarea value={stage.conditions.join("\n")} onChange={e=>patchStage({conditions:textList(e.target.value)})}/></> : <ul className="checklist">{stage.conditions.map((g,i)=>{const k=`${guide.id}-${stage.id}-${stage.goals.length+i}`;return <li key={k}><label><input type="checkbox" checked={!!checked[k]} onChange={()=>setChecked(c=>({...c,[k]:!c[k]}))}/><span>{g}</span></label></li>})}</ul>}
            <div className="condition-score"><strong>{stage.conditions.filter((_,i)=>checked[`${guide.id}-${stage.id}-${stage.goals.length+i}`]).length}<span> / {stage.conditions.length}</span></strong><small>符合條件</small></div>
          </article>

          <article className="card skills-card"><div className="card-heading"><span className="icon">⌁</span><div><small>GEM LINKS</small><h2>技能連線</h2></div></div>
            {stage.skills.length ? stage.skills.map((link,i)=><div className="skill-row" key={i}><span className="link-name">{i===0?"主力輸出":`技能組 ${i+1}`}</span><div>{link.map((s,x)=>editing?<span className="gem-edit" key={x}><select value={s.color} onChange={e=>patchStage({skills:stage.skills.map((row,ri)=>ri===i?row.map((gem,gi)=>gi===x?{...gem,color:e.target.value as Skill["color"]}:gem):row)})}><option value="blue">藍</option><option value="red">紅</option><option value="green">綠</option></select><input value={s.name} onChange={e=>patchStage({skills:stage.skills.map((row,ri)=>ri===i?row.map((gem,gi)=>gi===x?{...gem,name:e.target.value}:gem):row)})}/><button aria-label={`移除 ${s.name}`} onClick={()=>patchStage({skills:stage.skills.map((row,ri)=>ri===i?row.filter((_,gi)=>gi!==x):row)})}>×</button></span>:<span className={`gem ${s.color}`} key={x}><i>{s.color==="blue"?"B":s.color==="red"?"R":"G"}</i>{s.name}</span>)}</div></div>) : <p className="empty">尚未填寫技能連線。</p>}
            {editing && <button className="inline-button" onClick={()=>patchStage({skills:stage.skills.length?stage.skills.map((row,i)=>i===0?[...row,{name:"新寶石",color:"blue"}]:row):[[{name:"新寶石",color:"blue"}]]})}>＋ 新增寶石到主技能組</button>}
          </article>
          ...
          <article className="card gear-card"><div className="card-heading"><span className="icon">↗</span><div><small>UPGRADES</small><h2>裝備升級</h2></div></div>
            <div className="gear-table"><div className="gear-head"><span>部位</span><span>目前</span><span></span><span>下一步</span></div>{stage.gear.map((g,i)=><div className="gear-row" key={i}>{editing?<><input value={g.slot} onChange={e=>patchStage({gear:stage.gear.map((x,n)=>n===i?{...x,slot:e.target.value}:x)})}/><input value={g.current} onChange={e=>patchStage({gear:stage.gear.map((x,n)=>n===i?{...x,current:e.target.value}:x)})}/><span>→</span><input value={g.next} onChange={e=>patchStage({gear:stage.gear.map((x,n)=>n===i?{...x,next:e.target.value}:x)})}/></>:<><strong>{g.slot}</strong><span>{g.current}<small>{g.note}</small></span><b>→</b><span>{g.next}</span></>}</div>)}</div>
      ...
      {showLibrary && <div className="modal-backdrop" onMouseDown={()=>setShowLibrary(false)}><section className="modal" onMouseDown={e=>e.stopPropagation()}><div className="modal-head"><div><small>MY BUILD LIBRARY</small><h2>選擇攻略</h2></div><button onClick={()=>setShowLibrary(false)}>×</button></div><div className="guide-grid">{guides.map(g=><button key={g.id} onClick={()=>{setGuideId(g.id);setStageId(g.stages[0].id);setShowLibrary(false)}}><span className="guide-version">{g.version}</span><strong>{g.title}</strong><small>{g.archetype}</small><i>{g.stages.length} 個階段</i></button>)}<button className="add-card" onClick={newGuide}><strong>＋</strong><span>建立空白攻略</span></button></div></section></div>}
    </main>
  );
}
